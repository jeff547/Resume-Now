...
async def test_refresh(async_client: AsyncClient, test_user: tuple[User, str]):
    ...
    user = test_user[0]
    user_data = {"email": user.email, 'user_uid': str(user.id)}
    access_token = create_access_token(data=user_data, expire_seconds=0.01)
    refresh_token = create_refresh_token(data=user_data)
    time.sleep(0.01) # Let access token expire
    
    # Call expired access token
    r = await async_client.get('/users/self', headers= {"Authorization": f"Bearer {access_token}"})
    assert r.status_code == 403
    
    # Refresh access token
    r = await async_client.post("/token/refresh", params={'refresh_token': refresh_token})
    assert r.status_code == 200
...
async def test_invalid_refresh_token(async_client: AsyncClient, test_user: tuple[User, str]):
    ...
    user_data = {"email": user.email, 'user_uid': str(user.id)}
    access_token = create_access_token(data=user_data, expire_seconds=0.01)
    refresh_token = create_refresh_token(data=user_data, expires_days= 0)
    time.sleep(0.01) # Let access token expire
